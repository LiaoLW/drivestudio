...
def main(args):
    ...
    trainer = import_str(cfg.trainer.type)(
        **cfg.trainer,
        ...
        scene_aabb=dataset.get_aabb().reshape(2, 3), # axis-aligned bounding box
        device=device
    )
    ...
    for step in metric_logger.log_every(all_iters, cfg.logging.print_freq):
        ...
        if step % cfg.logging.vis_freq == 0 and cfg.logging.vis_freq > 0:
            ...
            if args.enable_neptune:
                for k, v in vis_frame_dict.items():
                    run["train/image_rendering/" + k].upload(File.as_image(v))
            del render_results
...
if __name__ == "__main__":
    ...
    parser.add_argument("--enable_neptune", default=True, help="enable neptune logging")
