...
class DeformableNodes(RigidNodes):
    def __init__(self, **kwargs):
        ...
        self.instances_embedding = torch.zeros(
            1, self.networks_cfg.embed_dim, device=self.device
        )
    ...
    def create_from_pcd(self, instance_pts_dict: Dict[str, torch.Tensor]) -> None:
        ...
        init_embedding = torch.rand(
            self.num_instances, self.networks_cfg.embed_dim, device=self.device
        )
        self.instances_embedding = Parameter(
            init_embedding
        )  # overrided the previous one

    def get_param_groups(self) -> Dict[str, List[Parameter]]:
        param_groups = self.get_gaussian_param_groups()
        param_groups[self.class_prefix + "embedding"] = [self.instances_embedding]
        param_groups[self.class_prefix + "deform_network"] = list(
            self.deform_network.parameters()
        )
        return param_groups
    ...
    def get_gaussians(self, cam: dataclass_camera) -> Dict[str, torch.Tensor]:
        ...
        if (
            self.ctrl_cfg.use_deformgs_for_nonrigid
            and self.step > self.ctrl_cfg.use_deformgs_after
        ):
            delta_xyz, delta_quat, delta_scale = self.get_deformation(
                local_means=self._means
            )
    ...
    def compute_reg_loss(self):
        ...
        if out_of_bound_losscfg is not None:
            ...
            if w > 0 and local_xyz_deformed is not None:
                ...
                loss_dict["out_of_bound_loss"] = (
                    torch.relu(local_xyz_deformed.abs() - per_pts_size / 2).mean() * w
                )
        return loss_dict
    ...
    def deform_gaussian_points(
        ...
        delta_xyz, delta_quat, delta_scale = self.deform_network(
            means, cur_normalized_time, nonrigid_embed
        )
