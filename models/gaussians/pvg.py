...
class PeriodicVibrationGaussians(VanillaGaussians):
    ...
    def register_normalized_timestamps(self, normalized_timestamps: int):
        ...
        self.train_time_scale = (
            self.ctrl_cfg.time_interval / self.normalized_time_interval
        )
    ...
    def create_from_pcd(
        ...
        self._taus = Parameter(
            (init_times * self.train_time_scale).to(self.device)
        )  # life peak
        self._velocity = Parameter(
            torch.zeros(self.num_points, 3).to(self.device)
        )  # vibration direction
        betas_init = torch.sqrt(
            torch.ones(self.num_points, 1).to(self.device) * self.ctrl_cfg.betas_init
        )
    ...
    def temporal_means(self):
        ...
        means = (
            self._means
            + self._velocity * torch.sin((self.cur_time - self._taus) * a) / a
        )
        if self.in_smooth:
    ...
    def get_marginal_t(self):
        return torch.exp(
            -0.5 * (self._taus - self.cur_time) ** 2 / self.get_scaling_t**2
        )
    ...
    def gamma(self):
        ...
        """
        with torch.no_grad():
            gamma = (self._means - self.scene_origin).norm(
                dim=-1
            ) * self.scene_scale - 1
    ...
    def after_train(
        ...
        with torch.no_grad():
            ...
            full_mask = torch.zeros(
                self.num_points, device=radii.device, dtype=torch.bool
            )
            ...
            if self.xys_grad_norm is None:
                self.xys_grad_norm = torch.zeros(
                    self.num_points, device=grads.device, dtype=grads.dtype
                )
                self.xys_grad_norm[self.filter_mask] = grads

                self.t_grad_accum = torch.zeros(
                    self.num_points, device=grads.device, dtype=grads.dtype
                )
                ...
                self.xys_grad_norm[full_mask] = (
                    grads[visible_mask] + self.xys_grad_norm[full_mask]
                )
                self.t_grad_accum[full_mask] = (
                    t_grads[visible_mask] + self.t_grad_accum[full_mask]
                )

            # update the max screen size, as a ratio of number of pixels
            if self.max_2Dsize is None:
                self.max_2Dsize = torch.zeros(
                    self.num_points, device=radii.device, dtype=torch.float32
                )
    ...
    def refinement_after(self, step, optimizer: torch.optim.Optimizer) -> None:
        ...
        with torch.no_grad():
            ...
            if do_densification:
                ...
                splits_t = (
                    torch.max(self.get_scaling_t, dim=1).values
                    > self.ctrl_cfg.densify_t_size_thresh
                ) & high_t_grads
                ...
                self._velocity = Parameter(
                    torch.cat([self._velocity.detach(), split_dirs, dup_dirs], dim=0)
                )
                self._taus = Parameter(
                    torch.cat([self._taus.detach(), split_taus, dup_taus], dim=0)
                )
                self._betas = Parameter(
                    torch.cat([self._betas.detach(), split_betas, dup_betas], dim=0)
                )
    ...
    def split_gaussians(self, split_mask, samps):
        ...
        new_betas = torch.log(
            self.get_scaling_t[split_mask].repeat(samps, 1) / size_fac
        )
        new_velocity = self._velocity[split_mask].repeat(samps, 1)
    ...
    def get_gaussians(self, cam: dataclass_camera) -> Dict:
        ...
        scaled_train_t = (
            self.normalized_timestamps[self.cur_frame] * self.train_time_scale
        )  # t2 in paper
        if self.training and (
            self.ctrl_cfg.enable_temporal_smoothing
            and random.random() < self.ctrl_cfg.smooth_probability
        ):
            self.in_smooth = True
            bound = (
                self.normalized_time_interval
                * self.ctrl_cfg.distribution_span
                * self.train_time_scale
            )
            self.cur_time = (
                scaled_train_t + uniform.Uniform(-bound, bound).sample((1,)).item()
            )  # t1 in paper
    ...
    def compute_reg_loss(self):
        ...
        if velocity_reg:
            if (self.cur_radii > 0).sum():
                ...
                loss_dict["velocity_reg"] = (
                    velocity_loss[self.cur_radii > 0].mean() * velocity_reg.w
                )
        return loss_dict
    ...
    def load_state_dict(self, state_dict: Dict, **kwargs) -> str:
        ...
        self._means = Parameter(
            torch.zeros((N,) + self._means.shape[1:], device=self.device)
        )
        self._scales = Parameter(
            torch.zeros((N,) + self._scales.shape[1:], device=self.device)
        )
        self._quats = Parameter(
            torch.zeros((N,) + self._quats.shape[1:], device=self.device)
        )
        self._features_dc = Parameter(
            torch.zeros((N,) + self._features_dc.shape[1:], device=self.device)
        )
        self._features_rest = Parameter(
            torch.zeros((N,) + self._features_rest.shape[1:], device=self.device)
        )
        self._opacities = Parameter(
            torch.zeros((N,) + self._opacities.shape[1:], device=self.device)
        )
        self._taus = Parameter(
            torch.zeros((N,) + self._taus.shape[1:], device=self.device)
        )
        self._betas = Parameter(
            torch.zeros((N,) + self._betas.shape[1:], device=self.device)
        )
        self._velocity = Parameter(
            torch.zeros((N,) + self._velocity.shape[1:], device=self.device)
        )
