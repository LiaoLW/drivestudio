...
class VanillaGaussians(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._features_rest = torch.zeros(
            1, num_sh_bases(self.sh_degree) - 1, 3, device=self.device
        )
    ...
    def create_from_pcd(
        self, init_means: torch.Tensor, init_colors: torch.Tensor
    ) -> None:
    ...
    def get_scaling(self):
        if self.ball_gaussians:
            if self.gaussian_2d:
                ...
                scaling = torch.cat(
                    [scaling, torch.zeros_like(scaling[..., :1])], dim=-1
                )
                return scaling
            else:
            ...
            if self.gaussian_2d:
                ...
                scaling = torch.cat(
                    [scaling[..., :2], torch.zeros_like(scaling[..., :1])], dim=-1
                )
                return scaling
            else:
    ...
    def compute_reg_loss(self):
        ...
        if sparse_reg:
            if (self.cur_radii > 0).sum():
                ...
                sparse_loss = (
                    -1
                    * (log_opacity + log_one_minus_opacity)[self.cur_radii > 0].mean()
                )
        ...
        if max_s_square_reg is not None and not self.ball_gaussians:
            loss_dict["max_s_square"] = (
                torch.mean((self.get_scaling.max(dim=1).values) ** 2)
                * max_s_square_reg.w
            )
        return loss_dict
