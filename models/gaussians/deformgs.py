...
class DeformableGaussians(VanillaGaussians):
    def __init__(self, **kwargs):
        ...
        self.deform_network = DeformNetwork(input_ch=3, **self.networks_cfg).to(
            self.device
        )
    ...
    def get_deformation(self, canonical_means) -> Tuple:
        ...
        normed_canonical_means = contract(
            canonical_means, self.bbox
        )  # range from 0 to 1
        ...
        delta_xyz, delta_quat, delta_scale = self.deform_network(
            normed_canonical_means.data, t + ast_noise
        )
