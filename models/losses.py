...
def reduce(
    ...
    if mask is not None:
        ...
        assert (
            loss.dim() == mask.dim()
        ), f"Expects loss.dim={loss.dim()} to be equal to mask.dim()={mask.dim()}"

    if reduction == 'mean':
        return loss.mean() if mask is None else (loss * mask).mean()
    elif reduction == 'mean_in_mask':
        return (
            loss.mean() if mask is None else (loss * mask).sum() / mask.sum().clip(1e-5)
        )
    elif reduction == 'sum':
...
class SafeBCE(autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[0]:
            ...
            grad_x = (
                torch.where(y == 0, 1 / (1 - x), -1 / x) * grad_output * (~(x == y))
            )
...
class DepthLoss(nn.Module):
    ...
    def _compute_depth_loss(
        ...
        if self.normalize:
            pred_depth = safe_normalize_depth(
                pred_depth[valid_mask], max_depth=max_depth
            )
    ...
    def __call__(
        ...
        depth_error = self._compute_depth_loss(
            pred_depth, gt_depth, self.upper_bound, hit_mask
        )
