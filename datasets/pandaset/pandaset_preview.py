...
def count_labels(base_path: str = 'data/pandaset/raw'):
            ...
            for pkl_file in os.listdir(scene_path):
                if pkl_file.endswith('.pkl'):
                    ...
                    with open(pkl_file_path, 'rb') as f:
                        ...
                        combined_label_counts = combined_label_counts.add(
...
def preview_camera_captures(
    ...
    for scene_id in tqdm(sorted(os.listdir(input_root))):
        ...
        images_all = sorted(
            [f for f in sorted(os.listdir(scene_folder)) if f.lower().endswith('.jpg')]
        )
        ...
        for frame_id in range(num_frames):
            ...
            front_left = cv2.imread(
                os.path.join(scene_folder, images_all[frame_id * 6 + 1])
            )
            front_right = cv2.imread(
                os.path.join(scene_folder, images_all[frame_id * 6 + 2])
            )
...
def preview_dynamic_masks(
    ...
    for scene_id in tqdm(sorted(os.listdir(input_root))):
        scene_folder = os.path.join(
            input_root, scene_id, "dynamic_masks", dynamic_class
        )
        ...
        # Get paths to each camera image folder
        images_all = sorted(
            [f for f in sorted(os.listdir(scene_folder)) if f.lower().endswith('.png')]
        )
        num_images = len(images_all)
        num_frames = num_images // 6
...
if __name__ == '__main__':
    ...
    cat_all_videos(
        keys=['camera_captures', '3dbox_vis', 'dynamic_masks', 'instances'],
        scene_id='all',
        fps=16,
    )
