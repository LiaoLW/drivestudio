...
class PandaSetProcessor(object):
        ...
        for frame_idx in tqdm(
            ...
            if "dynamic_masks" in self.process_keys:
                self.save_dynamic_mask(
                    scene_data, scene_idx, frame_idx, class_valid='all'
                )
                self.save_dynamic_mask(
                    scene_data, scene_idx, frame_idx, class_valid='human'
                )
                self.save_dynamic_mask(
                    scene_data, scene_idx, frame_idx, class_valid='vehicle'
                )
        ...
        if "objects" in self.process_keys:
            ...
            for pair in duplicated_id_pairs:
                ...
                if pair[0] not in instances_info:
                    ...
                    if len(
                        instances_info[pair[0]]['frame_annotations']['frame_idx']
    ...
    def save_pose(self, scene_data: Sequence, scene_idx, frame_idx):
        ...
        """
        lidar_poses = scene_data.lidar.poses[frame_idx]
        lidar_to_world = geometry._heading_position_to_mat(
            lidar_poses['heading'], lidar_poses['position']
        )
    ...
    def visualize_3dbox(self, scene_data: Sequence, scene_idx, frame_idx):
        ...
        for idx, cam in enumerate(self.cam_list):
            ...
            img_plotted = dump_3d_bbox_on_image(
                coords=lstProj2d, img=canvas, color=(255, 0, 0)
            )
            ...
            for idx, cam in enumerate(self.cam_list):
                ...
                if frame_idx in frame_instances:
                    ...
                    for obj_id in objects:
                        ...
                        o2w = instances_info[obj_id]['frame_annotations'][
                            'obj_to_world'
                        ][idx_in_obj]
                        o2w = np.array(o2w)
                ...
                img_path = (
                    f"{output_path}/" + f"{str(frame_idx).zfill(3)}_{str(idx)}.jpg"
                )
                Image.fromarray(img_plotted).save(img_path)

    def save_dynamic_mask(
        self, scene_data: Sequence, scene_idx, frame_idx, class_valid='all'
    ):
        ...
        for idx, cam in enumerate(self.cam_list):
            ...
            dynamic_mask_path = os.path.join(
                mask_dir, f"{str(frame_idx).zfill(3)}_{str(idx)}.png"
            )
    ...
    def save_objects(self, scene_data: Sequence, num_frames):
        ...
        for frame_idx in range(num_frames):
            ...
            for _, row in cuboids.iterrows():
                ...
                instances_info[str_id]['frame_annotations']['stationary'].append(
                    row["stationary"]
                )
