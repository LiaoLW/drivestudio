...
class HMR2_4dhuman(PHALP):
    ...
    def get_detections(
        ...
        # Pad bounding boxes
        pred_bbox_padded = expand_bbox_to_aspect_ratio(
            pred_bbox, self.cfg.expand_bbox_shape
        )
...
def run_4DHumans(
    ...
    for cam_id in camera_list:
        ...
        if not os.path.exists(cached_video_path):
            logger.info(
                f"Cached video not found at {cached_video_path}, we will create it"
            )
    ...
    if not verbose:
        ...
        files_to_remove = [
            os.path.join(temp_dir, 'phalp_output', f'PHALP_{i}.mp4')
