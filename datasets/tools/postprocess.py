    ...
    for i in range(n):
        if not mask[i]:
            ...
            if prev_index >= 0 and next_index < n:
                ...
                if not is_rot_mat:
                    features[i] = (1 - interp_factor) * features[
                        prev_index
                    ] + interp_factor * features[next_index]
                else:
                    features[i] = interpolate_matrix(
                        features[prev_index], features[next_index], interp_factor
                    )
...
def detect_breaks_mask(bool_sequence):
    ...
    if start_false_sequence and last_true_index < len(bool_sequence) - 1:
        mask[last_true_index + 1 : len(bool_sequence)] = [False] * (
            len(bool_sequence) - last_true_index - 1
        )
...
def match_and_postprocess(
    ...
    for cam_id in camera_list:
        ...
        for fi, pred_track_f in enumerate(raw_pred_track.values()):
            for i_, tid in enumerate(pred_track_f["tid"]):
                ...
                parsed_pred_track[tid]["tracked_bbox"][fi] = torch.from_numpy(
                    pred_track_f["bbox"][i_]
                )
                parsed_pred_track[tid]["smpl"]["global_orient"][fi] = torch.tensor(
                    pred_track_f["smpl"][i_]["global_orient"]
                )
                parsed_pred_track[tid]["smpl"]["body_pose"][fi] = torch.tensor(
                    pred_track_f["smpl"][i_]["body_pose"]
                )
                parsed_pred_track[tid]["smpl"]["betas"][fi] = torch.tensor(
                    pred_track_f["smpl"][i_]["betas"]
                )
                parsed_pred_track[tid]["camera"][fi] = torch.tensor(
                    pred_track_f["camera"][i_]
                )
        ...
        for fi, gt_track_f in enumerate(raw_gt_track.values()):
            for i_, tid in enumerate(gt_track_f["extra_data"]["gt_track_id"]):
                ...
                parsed_gt_track[tid]["tracked_bbox"][fi] = torch.tensor(
                    gt_track_f["gt_bbox"][i_]
                )
        ...
        for pred_tid in parsed_pred_track.keys():
            ...
            for gt_tid in parsed_gt_track.keys():
                ...
                for fi in range(num_f):
                    if (
                        parsed_pred_track[pred_tid]["valids"][fi]
                        and parsed_gt_track[gt_tid]["valids"][fi]
                    ):
                        iou += compute_iou(
    ...
    for cam_id in camera_list:
        ...
        if len(_matches) > 0:
            ...
            for gt_tid in valid_gt_tids:
                ...
                for fi in range(num_f):
                    if (
                        parsed_gt_track[gt_tid]["valids"][fi]
                        and parsed_pred_track[matched_pred_tid]["valids"][fi]
                    ):
                        collector[gt_tid]["smpl"]["global_orient"][cam_id, fi] = (
                            parsed_pred_track[matched_pred_tid]["smpl"][
                                "global_orient"
                            ][fi].clone()
                        )
                        collector[gt_tid]["smpl"]["body_pose"][cam_id, fi] = (
                            parsed_pred_track[matched_pred_tid]["smpl"]["body_pose"][
                                fi
                            ].clone()
                        )
                        collector[gt_tid]["smpl"]["betas"][cam_id, fi] = (
                            parsed_pred_track[matched_pred_tid]["smpl"]["betas"][
                                fi
                            ].clone()
                        )
                        collector[gt_tid]["camera"][cam_id, fi] = parsed_pred_track[
                            matched_pred_tid
                        ]["camera"][fi].clone()
            ...
            for fi in range(num_f):
                ...
                for gt_tid in valid_gt_tids:
                    if collector[gt_tid]["valid_mask"][cam_id, fi]:
                        ...
                        fi_info["smpl"].append(
                            {
                                "global_orient": collector[gt_tid]["smpl"][
                                    "global_orient"
                                ][cam_id, fi].numpy(),
                                "body_pose": collector[gt_tid]["smpl"]["body_pose"][
                                    cam_id, fi
                                ].numpy(),
                                "betas": collector[gt_tid]["smpl"]["betas"][
                                    cam_id, fi
                                ].numpy(),
                            }
                        )
                        fi_info["camera"].append(
                            collector[gt_tid]["camera"][cam_id, fi].numpy()
                        )
                pkl_dict[fi] = fi_info
    ...
    for cam_id in camera_list:
        for gt_tid in valid_gt_tids:
            need_fill = collector[gt_tid]["2DBox_appear_mask"][cam_id] & (
                ~collector[gt_tid]["matched_mask"][cam_id]
            )
            if (
                need_fill.any()
                and not collector[gt_tid]["matched_mask"][cam_id].sum() == 0
            ):
                appear_mask = collector[gt_tid]["2DBox_appear_mask"][cam_id]
                mask = collector[gt_tid]["valid_mask"][cam_id][appear_mask]
                collector[gt_tid]["matched_mask"][cam_id] = (
                    collector[gt_tid]["matched_mask"][cam_id] | appear_mask
                )

                feature_masked = collector[gt_tid]["smpl"]["global_orient"][
                    cam_id, appear_mask
                ]
                feature_full = interpolate_features(
                    mask, feature_masked, is_rot_mat=True
                )
                collector[gt_tid]["smpl"]["global_orient"][
                    cam_id, appear_mask
                ] = feature_full

                feature_masked = collector[gt_tid]["smpl"]["body_pose"][
                    cam_id, appear_mask
                ]
                feature_full = interpolate_features(
                    mask, feature_masked, is_rot_mat=True
                )
                collector[gt_tid]["smpl"]["body_pose"][
                    cam_id, appear_mask
                ] = feature_full
    ...
    for gt_tid in valid_gt_tids:
        ...
        best_cam_id = matched_cam_id * (matched_sum == 1) + max_cam_id * (
            matched_sum > 1
        )
        ...
        merged_collector[gt_tid]["smpl"]["global_orient"][mask] = collector[gt_tid][
            "smpl"
        ]["global_orient"][:, mask][
            masked_best_cam_id, torch.arange(masked_best_cam_id.shape[0])
        ]
        merged_collector[gt_tid]["smpl"]["body_pose"][mask] = collector[gt_tid]["smpl"][
            "body_pose"
        ][:, mask][masked_best_cam_id, torch.arange(masked_best_cam_id.shape[0])]
        merged_collector[gt_tid]["smpl"]["betas"][mask] = collector[gt_tid]["smpl"][
            "betas"
        ][:, mask][masked_best_cam_id, torch.arange(masked_best_cam_id.shape[0])]
        merged_collector[gt_tid]["camera"] = collector[gt_tid]["camera"]
        merged_collector[gt_tid]["2DBox_appear_mask"] = collector[gt_tid][
            "2DBox_appear_mask"
        ]
    ...
    for gt_tid in valid_gt_tids:
        need_fill = merged_collector[gt_tid]["must_appear_mask"] & (
            ~merged_collector[gt_tid]["matched_mask"]
        )
        if need_fill.any():
            ...
            feature_masked = merged_collector[gt_tid]["smpl"]["global_orient"][
                final_complete_mask
            ]
            feature_full = interpolate_features(mask, feature_masked, is_rot_mat=True)
            merged_collector[gt_tid]["smpl"]["global_orient"][
                final_complete_mask
            ] = feature_full

            feature_masked = merged_collector[gt_tid]["smpl"]["body_pose"][
                final_complete_mask
            ]
            feature_full = interpolate_features(mask, feature_masked, is_rot_mat=True)
            merged_collector[gt_tid]["smpl"]["body_pose"][
                final_complete_mask
            ] = feature_full

            feature_masked = merged_collector[gt_tid]["smpl"]["betas"][
                final_complete_mask
            ]
            feature_full = interpolate_features(mask, feature_masked)
            merged_collector[gt_tid]["smpl"]["betas"][
                final_complete_mask
            ] = feature_full
            ...
            feature_masked = merged_collector[gt_tid]["selected_cam_idx"][
                final_complete_mask
            ]
            feature_full = interpolate_features(mask, feature_masked)
            merged_collector[gt_tid]["selected_cam_idx"][
                final_complete_mask
            ] = feature_full.round()
    ...
    # update valid_mask
    for gt_tid in valid_gt_tids:
        merged_collector[gt_tid]["valid_mask"] = merged_collector[gt_tid][
            "must_appear_mask"
        ].clone()
            ...
            for fi in range(num_f):
                ...
                for gt_tid in valid_gt_tids:
                    if merged_collector[gt_tid]["2DBox_appear_mask"][cam_id, fi]:
                        ...
                        fi_info["smpl"].append(
                            {
                                "global_orient": merged_collector[gt_tid]["smpl"][
                                    "global_orient"
                                ][fi].numpy(),
                                "body_pose": merged_collector[gt_tid]["smpl"][
                                    "body_pose"
                                ][fi].numpy(),
                                "betas": merged_collector[gt_tid]["smpl"]["betas"][
                                    fi
                                ].numpy(),
                            }
                        )
                        fi_info["camera"].append(
                            merged_collector[gt_tid]["camera"][cam_id, fi].numpy()
                        )
                pkl_dict[fi] = fi_info
