...
NUSCENES_DYNAMIC_CLASSES = (
    NUSCENES_NONRIGID_DYNAMIC_CLASSES + NUSCENES_RIGID_DYNAMIC_CLASSES
)
...
class NuScenesProcessor(object):
    ...
    def convert_one(self, scene_idx):
        ...
        if "objects" in self.process_keys:
            ...
            instances_info_save_path = (
                f"{self.save_dir}/{str(scene_idx).zfill(3)}/instances"
            )
            ...
            if "objects_vis" in self.process_keys:
                self.visualize_dynamic_objects(
                    scene_data, scene_idx, instances_info, frame_instances
                )
                print(
                    f"Processed objects visualization for scene {str(scene_idx).zfill(3)}"
                )
    ...
    def convert_one_interpolated(self, scene_idx):
        ...
        interpolated_timestamps = self.get_interpolated_timestamps(
            keyframe_timestamps, self.interpolate_N
        )
    ...
    def get_percam_meta(self, scene_data):
        ...
        cams_mata = {
            cam_name: {"timestamps": [], "is_key_frame": [], "token": []}
            for cam_name in self.cam_list
        }

        for cam_name in self.cam_list:
            cur_img_data = self.nusc.get(
                'sample_data', first_sample_record['data'][cam_name]
            )
            while True:
        ...
        while True:
            for cam_idx, cam_name in enumerate(self.cam_list):
                cam_data = self.nusc.get(
                    'sample_data', curr_sample_record['data'][cam_name]
                )
    ...
    def save_image_interpolated(self, scene_data, scene_idx, timestamps: np.array):
        ...
        for cam_idx, cam_name in enumerate(self.cam_list):
            ...
            closest_tokens = self.find_closest_img_tokens(
    ...
    def save_dynamic_mask(self, scene_data, scene_idx, class_valid='all'):
        ...
        while True:
            for cam_idx, cam_name in enumerate(self.cam_list):
                ...
                valid_anns = [
                    ann for ann in anns if ann['category_name'] in VALID_CLASSES
                ]
    ...
    def save_dynamic_mask_interpolated(
        self, scene_data, scene_idx, timestamps, instances_info, class_valid='all'
    ):
        ...
        for frame_idx, timestamp in enumerate(timestamps):
            for cam_idx, cam_name in enumerate(self.cam_list):
                ...
                closest_token = self.find_closest_img_tokens(
                    scene_data, [timestamp], cam_name
                )[0]
                ...
                for obj_id in objects:
                    ...
                    idx_in_obj = obj_info['frame_annotations']['frame_idx'].index(
                        frame_idx
                    )
                    o2w = np.array(
                        obj_info['frame_annotations']['obj_to_world'][idx_in_obj]
                    )
                    length, width, height = obj_info['frame_annotations']['box_size'][
                        idx_in_obj
                    ]
        ...
        while True:
            ...
            frame_instances[key_frame_idx].extend(
                [
                    ann['instance_token']
                    for ann in anns
                    if ann['category_name'] in NUSCENES_DYNAMIC_CLASSES
                ]
            )
    ...
    def visualize_dynamic_objects(
        self, scene_data, scene_idx, instances_info, frame_instances
    ):
            ...
            for cam_idx, cam_name in enumerate(self.cam_list):
                ...
                if len(objects) > 0:
                    ...
                    for obj_id in objects:
                        ...
                        o2w = np.array(
                            instances_info[obj_id]['frame_annotations']['obj_to_world'][
                                idx_in_obj
                            ]
                        )
                        ...
                        corners_ego = corners_world - np.array(
                        ...
                        corners_cam = corners_ego - np.array(
                        ...
                        # Project to 2D
                        corners_2d = view_points(
                            corners_cam, camera_intrinsic, normalize=True
                        )
            ...
            for cam_idx, cam_name in enumerate(self.cam_list):
                ...
                if len(objects) > 0:
                    ...
                    for obj_id in objects:
                        ...
                        length, width, height = obj_info['frame_annotations'][
                            'box_size'
                        ][idx_in_obj]
                        corners = get_corners(length, width, height)
    ...
    def create_folder(self):
        ...
        for i in id_list:
            ...
            if "lidar" in self.process_keys:
                ...
                os.makedirs(
                    f"{self.save_dir}/{str(i).zfill(3)}/lidar_pose", exist_ok=True
                )
            if "dynamic_masks" in self.process_keys:
                os.makedirs(
