...
def layout_nuplan(imgs: List[np.ndarray], cam_names: List[str]) -> np.ndarray:
    ...
    for scene_id in tqdm(sorted(os.listdir(input_root))):
        ...
        for frame_id in range(num_frames):
            ...
            for cam_id in range(8):
                img = cv2.imread(
                    os.path.join(scene_folder, images_all[frame_id * 8 + cam_id])
                )
                imgs.append(img)
                cam_names.append(
                    f"CAM_{['F0', 'L0', 'R0', 'L1', 'R1', 'L2', 'R2', 'B0'][cam_id]}"
                )
