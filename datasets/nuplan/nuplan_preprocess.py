...
class NuPlanProcessor(object):
    ...
    def __init__(
        ...
        print(
            "We will process the first {} frames each scene".format(
                self.max_frame_limit
            )
        )
        # the lidar data is collected at 20Hz, we need to downsample to 10Hz to match the camera data
        self.lidar_idxs = range(
            self.start_frame_idx, self.start_frame_idx + self.max_frame_limit * 2, 2
        )
    ...
    def convert_one(self, scene_log_name):
        ...
        no_shift_time_diff = [
            abs(lidar_timestamp - timestamp) for timestamp in images_timestamps
        ]
        ...
        if "dynamic_masks" in self.process_keys:
            self.save_dynamic_mask(
                log_db, lidar_idxs, valid_classes=NUPLAN_DYNAMIC_CLASSES, dir_name='all'
            )
            self.save_dynamic_mask(
                log_db,
                lidar_idxs,
    ...
    def save_lidar(self, log_db: NuPlanDB, lidar_idxs: List[int]):
        ...
        for frame_idx, lidar_idx in enumerate(lidar_idxs):
            ...
            lidar_save_path = (
                f"{self.save_dir}/{log_db.log_name}/lidar/{str(frame_idx).zfill(3)}.bin"
            )
    ...
    def save_dynamic_mask(
        ...
        for frame_idx, lidar_idx in enumerate(lidar_idxs):
            ...
            objects = [
                obj for obj in objects_generator if obj.category in valid_classes
            ]
            ...
            for channel in self.cam_list:
                ...
                for obj in objects:
                    ...
                    corners_ego = (
                        world_to_ego[:3, :3] @ corners_world + world_to_ego[:3, 3:4]
                    )
                    ...
                    corners_2d, _ = project_camera_points_to_image(
                        corners_cam.T, intrinsics[channel]
                    )
    ...
    def save_objects(self, log_db: NuPlanDB, lidar_idxs: List[int]):
        ...
        for frame_idx, lidar_idx in enumerate(lidar_idxs):
            ...
            objects = [
                ...
                if obj.category in NUPLAN_DYNAMIC_CLASSES
            ]
            ...
            for obj in objects:
                ...
                instances_info[obj_id]['frame_annotations']['box_size'].append(
                    [l, w, h]
                )
            ...
            for cam_id, image in enumerate(images):
                ...
                if len(objects) == 0:
                    ...
                    for obj_id in objects:
                        ...
                        l, w, h = instances_info[obj_id]['frame_annotations'][
                            'box_size'
                        ][idx_in_obj]
                        ...
                        ego_to_cam = np.linalg.inv(extrinsics[channel])
                        corners_cam = (
                            ego_to_cam[:3, :3] @ corners_ego + ego_to_cam[:3, 3:4]
                        )
                ...
                img_path = os.path.join(
                    output_path, f"{str(frame_idx).zfill(3)}_{cam_id}.jpg"
                )
    ...
    def create_folder(self):
        ...
        for scene_log_name in id_list:
            if "images" in self.process_keys:
                ...
                os.makedirs(
                    f"{self.save_dir}/{scene_log_name}/sky_masks", exist_ok=True
                )
            if "pose" in self.process_keys:
                os.makedirs(f"{self.save_dir}/{scene_log_name}/ego_pose", exist_ok=True)
            if "calib" in self.process_keys:
                os.makedirs(
                    f"{self.save_dir}/{scene_log_name}/extrinsics", exist_ok=True
                )
                os.makedirs(
                    f"{self.save_dir}/{scene_log_name}/intrinsics", exist_ok=True
                )
            if "lidar" in self.process_keys:
                os.makedirs(f"{self.save_dir}/{scene_log_name}/lidar", exist_ok=True)
            if "dynamic_masks" in self.process_keys:
                os.makedirs(
                    f"{self.save_dir}/{scene_log_name}/dynamic_masks/all", exist_ok=True
                )
                os.makedirs(
            ...
            if "objects" in self.process_keys:
                os.makedirs(
                    f"{self.save_dir}/{scene_log_name}/instances", exist_ok=True
                )
                if "objects_vis" in self.process_keys:
                    os.makedirs(
